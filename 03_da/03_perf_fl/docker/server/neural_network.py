...
def prepare_train_data(dataset, edge_map):
    dataset["time_of_day"] = dataset["time"] / (24*60*60) #normalizing it for learning
    labels = dataset["rel_speed"].copy()

    features = np.zeros((len(dataset), len(edge_map)+1)) #one-hot-encoded parking_id | time_of_day
    i = 0
...
class NeuralNetwork:
    def _create_model(self):
        ...
        self.model.build(input_shape=(None,1230))
